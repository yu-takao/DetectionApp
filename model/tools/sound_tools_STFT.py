...
def load_sound_file(wav_name, mono=True, channel=0):
    ...
    """
    # S3 パス (s3://bucket/key) にも対応する
    if isinstance(wav_name, str) and wav_name.startswith("s3://"):
        # s3://bucket/key... をパース
        _, _, bucket_and_key = wav_name.partition("s3://")
        bucket, _, key = bucket_and_key.partition("/")
        s3 = boto3.client("s3")
        obj = s3.get_object(Bucket=bucket, Key=key)
        body = obj["Body"].read()
        bio = BytesIO(body)
        # 強制的にモノラル変換（マルチチャネルだと 3 次元配列になり、後段の処理と合わないため）
        return librosa.load(bio, sr=22050, mono=True)

    # 通常のローカルファイルパス
    return librosa.load(wav_name, sr=22050, mono=True)


def get_magnitude_scale(file, n_fft=1024, hop_length=520):
    """
...
def extract_signal_features(signal,sr,n_mels, frames, n_fft, hop_length):
# def extract_signal_features(signal, n_fft, hop_length):
    """
    ...
    log_mel_spectrogram = librosa.power_to_db(mel_spectrogram, ref=np.max) #melスケール
    
    # # Compute the short-time Fourier transform of the signal:
    # stft = librosa.stft(signal, n_fft=n_fft, hop_length=hop_length) #melスケール無し
   
    # # Map the magnitude to a decibel scale:
    # log_stft_spectrogram = librosa.amplitude_to_db(np.abs(stft), ref=np.max) #melスケール無し
    
    # Map the magnitude to a decibel scale:
    # dB = librosa.amplitude_to_db(np.abs(stft), ref=np.max)
    
    # # Generate an array of vectors as features for the current signal:
    # features_vector_size = log_stft_spectrogram.shape[1] - frames + 1 #melスケール無し
    features_vector_size = log_mel_spectrogram.shape[1] - frames + 1 #melスケール
    ...
    for t in range(frames):
        # features[:, n_mels * t: n_mels * (t + 1)] = log_stft_spectrogram[:, t:t + features_vector_size].T #melスケール無し
        features[:, n_mels * t: n_mels * (t + 1)] = log_mel_spectrogram[:, t:t + features_vector_size].T #melスケール
        
    return features

def generate_dataset(files_list,n_mels, frames, n_fft, hop_length):
    """
    ...
    for index in tqdm(range(len(files_list)), desc='Extracting features'):
        ...
        # 特徴量をリストに追加
        if features.shape[0] > 0:  # 空でない場合のみ追加
            features_list.append(features)
    
    # 全ての特徴量を連結
    if features_list:
        dataset = np.vstack(features_list)
    else:
        dataset = np.empty((0, dims), np.float32)
